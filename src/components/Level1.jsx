...
export default function Level1({ onComplete, onRestart }) {
  ...
  return (
    <div className="level1-root">
      ...
      {substage === SUBSTAGE.ENDING && (
        <EndingScreen refusalCount={refusalCount} onRefuse={() => {
          const next = refusalCount + 1;
          if (next >= 3) {
            onRestart?.();
          } else {
            setRefusalCount(next);
          }
        }} onNext={() => setSubstage(SUBSTAGE.ETHICAL)} />
      )}

      {substage === SUBSTAGE.ETHICAL && (
        <EthicalDilemma level={1} onDone={onComplete} />
      )}
    </div>
  );
}
...
function EndingScreen({ onNext, refusalCount = 0, onRefuse }) {
  const [phase, setPhase] = useState("breach");
  const [visible, setVisible] = useState(false);
  const [userIP, setUserIP] = useState("192.0.2.44");
  const [location, setLocation] = useState("SIMULATED CITY, INDIA");
  ...
  // Safety: breach information is simulated; the game never collects real IP/location data.
  useEffect(() => {
            ...
            <div className="l1-breach-narrator">
              ...
              {breachLine < NARRATOR_LINES_BREACH.length - 1 ? (
                ...
                <div className="l1-breach-actions">
                  ...
                  >
                    REFUSE {refusalCount < 3 ? `(${refusalCount + 1}/3)` : ""}
                  </button>
                  <button
