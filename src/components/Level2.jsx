...
export default function Level2({ onComplete }) {
  ...
  return (
    <div className="level2-root">
      <img src="/assets/placeholder.png" alt="" className="level2-bg" />
      <div className="level2-overlay" />

      {substage === SUBSTAGE.LANDING && (
        <div className={`l2-logo-wrap ${logoAnimDone ? "logo-topleft" : "logo-center"}`}>
          <img src="/assets/Trinetra_logo.png" alt="Trinetra" className="l2-logo-img" />
        </div>
      )}
      ...
      {substage === SUBSTAGE.ENDING && gameResult && (
        <EndingScreen 
          result={gameResult} 
          onRestart={() => setSubstage(SUBSTAGE.GAME)}
          onContinue={() => setSubstage(SUBSTAGE.ETHICAL)}
        />
      )}

      {substage === SUBSTAGE.ETHICAL && (
        <EthicalDilemma level={2} onDone={() => onComplete?.()} />
      )}
...
function EndingScreen({ result, onRestart, onContinue }) {
